...
property string city
    ...
    readonly property string fajr: prayerTimes?.Fajr ?? "--:--"
    readonly property string sunriseTime: prayerTimes?.Sunrise ?? "--:--"
    readonly property string dhuhr: prayerTimes?.Dhuhr ?? "--:--"
    readonly property string asr: prayerTimes?.Asr ?? "--:--"
    readonly property string maghrib: prayerTimes?.Maghrib ?? "--:--"
    readonly property string isha: prayerTimes?.Isha ?? "--:--"
    ...
    function fetchPrayerTimes(): void {
        ...
        const method = 14;
        const url = `https://api.aladhan.com/v1/timings?latitude=36.36&longitude=43.10&method=${method}`;
        ...
        Requests.get(url, text => {
            ...
            console.warn("Prayer times API failed:", err);
        });
    }
