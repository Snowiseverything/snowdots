...
CustomMouseArea {
    ...
    onPositionChanged: event => {
        ...
        if (panels.sidebar.offsetScale === 1) {
            ...
            // Show sidebar on top-right or bottom-right hover
            if (!visibilities.sidebar) {
                const atRightEdge = x > width - panels.sidebar.implicitWidth - Config.border.minThickness;
                const inNotifs = withinPanelHeight(panels.notifications, x, y);
                const inUtils = withinPanelHeight(panels.utilities, x, y);
                if (atRightEdge && (inNotifs || inUtils))
